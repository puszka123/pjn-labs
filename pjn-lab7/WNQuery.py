...
class WNQuery:
    ...
    def traceRelationMine(self, wnid, pos, rel, parent):
        if not parent.synonyms:
            parent.synonyms = ["{0}:{1}".format(i.literal, i.sense) for i in self.dat(pos)[parent.id].synonyms]
            parent.definition = "{0}".format(self.dat(pos)[parent.id].definition)
        ids = self.lookUpRelation(wnid, pos, rel)
        parent.add_children(ids)
        for child in parent.children:
            child.synonyms = ["{0}:{1}".format(i.literal, i.sense) for i in self.dat(pos)[child.id].synonyms]
            child.definition = "{0}".format(self.dat(pos)[child.id].definition)
        for i in ids:
            self.traceRelationMine(i, pos, rel, parent.get_child_by_id(i))  # recurse on target
